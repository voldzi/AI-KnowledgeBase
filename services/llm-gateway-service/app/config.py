...
def _get(env: Mapping[str, str], key: str, default: str) -> str:
    return _get_any(env, (key,), default)


def _get_any(env: Mapping[str, str], keys: tuple[str, ...], default: str) -> str:
    for key in keys:
        value = env.get(key)
...
def load_settings(env: Mapping[str, str] | None = None) -> Settings:
    ...
    default_provider = _get_any(
        source,
        ("AKL_LLM_DEFAULT_PROVIDER", "LLM_GATEWAY_PROVIDER"),
        "mock",
    ).strip().lower()
    ...
    default_embedding_model = _get_any(
        source,
        (
            "AKL_LLM_DEFAULT_EMBEDDING_MODEL",
            "LLM_GATEWAY_DEFAULT_EMBEDDING_MODEL",
            "AKL_OLLAMA_EMBEDDING_MODEL",
        ),
        "bge-m3",
    )
    ...
    return Settings(
        ...
        allow_model_pull=_parse_bool(
            _get_any(source, ("AKL_LLM_ALLOW_MODEL_PULL", "LLM_GATEWAY_ALLOW_MODEL_PULL"), "false")
        ),
        allow_model_delete=_parse_bool(
            _get_any(source, ("AKL_LLM_ALLOW_MODEL_DELETE", "LLM_GATEWAY_ALLOW_MODEL_DELETE"), "false")
        ),
