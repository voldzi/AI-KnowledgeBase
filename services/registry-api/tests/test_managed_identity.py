...
def access_projection(subject_id=SUBJECT, *, active=True):
    now = time.time()
    baseline = {
        "entitlementId": "system:akb:employee-baseline", "definitionVersion": "capabilities-1.12.4",
    ...
    return {
        ...
        "catalogVersion": "capabilities-1.12.4", "generatedAt": datetime.fromtimestamp(now - 1, timezone.utc).isoformat(),
