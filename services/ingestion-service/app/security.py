...
def require_service_auth(request: Request, settings: Settings) -> None:
    ...
    if not authorization or not authorization.startswith("Bearer "):
        raise IngestionError("UNAUTHORIZED", "Bearer token is required", status_code=401)

    token = authorization.removeprefix("Bearer ").strip()
    if settings.auth_mode == "bearer":
        if not settings.service_token or token != settings.service_token:
            raise IngestionError("FORBIDDEN", "Invalid service token", status_code=403)
...
def subject_for_request(request: Request, settings: Settings) -> str:
    return auth_context_for_request(request, settings).subject_id
...
def _verified_oidc_claims(token: str, settings: Settings) -> dict[str, Any]:
    if not settings.oidc_issuer or not settings.oidc_audience or not settings.oidc_jwks_url:
        raise IngestionError("AUTH_CONFIG_INVALID", "OIDC verification is not configured", status_code=503)
    try:
        ...
        raise IngestionError("UNAUTHORIZED", "Invalid bearer token", status_code=401) from exc
    return claims
