...
""")
    ...
    def test_prebuilt_import_is_closed_and_production_build_is_skipped(self) -> None:
        ...
        self.assertIn('AKB_RELEASE_CACHE_FROM_SHA: ${{ inputs.forward_fix_from_sha }}', workflow)
        self.assertIn('AKB_RELEASE_CACHE_FROM_SHA must be empty or a full lowercase Git SHA.', publisher)
        self.assertIn('Production cache source must be an ancestor of the release SHA.', publisher)
        self.assertIn('--cache-from "type=registry,ref=$registry/$owner/akb-$service:$cache_sha"', publisher)
        ...
        self.assertIn('local_registry_tag="git.home.cz/akb/akb-${service}:${RELEASE_SHA}"', workflow)
        self.assertIn('[[ "${local_reference}" == "${reference}" ]]', workflow)
        self.assertNotIn('docker pull "${reference}"', workflow)
