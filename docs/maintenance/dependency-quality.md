...
3. Run `scripts/ci/quality_dependencies.py`. It fails on a vulnerable or stale
   direct Node package, stale Python lock, version mismatch between CI/local
   Docker/production Docker, or a stale Docling release. It writes the audit
   result and CycloneDX SBOM under `.artifacts/quality`.
4. Run focused tests and build each affected production image from the
   repository root with its production Dockerfile.
5. Run the common Docker Desktop acceptance suite. Promote the exact candidate
...
part of CI. A separate daily workflow detects newly published versions and
newly disclosed vulnerabilities even when no source code changes.
