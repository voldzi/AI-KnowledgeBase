...
| DW-14 | RAG | Open `/chat`, start or select an AKB Assistant thread, and ask a question that should be answered from imported documents. | Answer is stored in the active thread, includes citations, opens source context in the chat/assistant citation viewer, and the document action opens the signed source file in a new tab while preserving the originating assistant/chat state. |
