...
| DW-14 | RAG | Ask a question that should be answered from imported documents. | Answer includes citations, opens source context in the chat/assistant citation viewer, and the document action opens the document detail viewer with the cited chunk in a new tab while preserving the originating assistant/chat state. |
