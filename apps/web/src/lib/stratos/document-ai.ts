...
export interface StratosBudgetUploadConfirmResult {
  document_id: string;
...
function strictStringList(value: unknown, field: string): string[] {
  ...
  if (!Array.isArray(value) || value.some((item) => typeof item !== "string" || !item.trim())) {
    budgetContractError(`${field} must be an array of non-empty strings.`);
  }
...
function strictOptionalString(body: Record<string, unknown>, field: string): string | null {
  ...
  if (typeof value !== "string" || !value.trim()) {
    budgetContractError(`${field} must be a non-empty string or null.`);
  }
  return value.trim();
}
