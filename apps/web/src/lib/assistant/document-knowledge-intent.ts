...
const DEADLINE_SIGNAL = /\b(?:do\s+kdy|od\s+kdy|jak\s+dlouho)\b|\bkdy\s+(?:musim|musime|musi|museji|mame|ma|je|jsou|se)\b|\b(?:jaka|jaky|jake)(?:\s+\w+){0,2}\s+(?:lhut\w*|termin\w*|periodicit\w*|platnost\w*|ucinnost\w*)\b/;
